...
export async function registerUser(formData: FormData) {
  try {
    ...
    if (!session?.user) {
      return {
        success: false,
        message: "Debes iniciar sesión para enviar el formulario.",
      };
    }
    ...
    if (existingSubmission) {
      return {
        success: false,
        message: "Ya has enviado un formulario anteriormente.",
      };
    }
    ...
    const preferencesData = {
      ...
      socialPreference: safeParseInt(
        formData.get("socialPreference") as string
      ),
      hobbyTime: formData.get("hobbyTime"),
      honestyImportance: safeParseInt(
        formData.get("honestidadImportance") as string
      ),
      loyaltyImportance: safeParseInt(
        formData.get("lealtadImportance") as string
      ),
      kindnessImportance: safeParseInt(
        formData.get("bondadImportance") as string
      ),
      respectImportance: safeParseInt(
        formData.get("respetoImportance") as string
      ),
      openMindednessImportance: safeParseInt(
        formData.get("apertura mentalImportance") as string
      ),
      independenceImportance: safeParseInt(
        formData.get("independenciaImportance") as string
      ),
      ambitionImportance: safeParseInt(
        formData.get("ambicionImportance") as string
      ),
      creativityImportance: safeParseInt(
        formData.get("creatividadImportance") as string
      ),
      humorImportance: safeParseInt(formData.get("humorImportance") as string),
      authenticityImportance: safeParseInt(
        formData.get("autenticidadImportance") as string
      ),
      empathyImportance: safeParseInt(
        formData.get("empatiaImportance") as string
      ),
      closenessEase: safeParseInt(formData.get("closenessEase") as string),
      conflictResolution: formData.get("conflictResolution"),
      attentionToDetail: safeParseInt(
        formData.get("attentionToDetail") as string
      ),
    ...
    try {
      await mongoSession.withTransaction(async () => {
        ...
        await textEmbeddings.insertOne(embeddingsData, {
    ...
    return {
      success: true,
      message:
        "¡Registro exitoso! Te contactaremos con tu pareja el 14 de febrero.",
    };
  } catch (error) {
...
export async function getMatches(userId: string): Promise<MatchResponse> {
  try {
    ...
    const parejaMatches = await Promise.all(
      (userMatches.pareja || []).map(
        async (match: { id: ObjectId; score: number }) =>
